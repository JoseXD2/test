...
function noteMiss(id, noteData, noteType, isSustainNote)
	if noteType == 'Static Note' then
		-- put something here if you want
		makeAnimatedLuaSprite('image', 'StaticLUL', -10, -10);
		setObjectCamera('image', 'other');
		addAnimationByPrefix('image', 'bop', 'bop', 24, true)
		objectPlayAnimation('image', 'bop', false);
		setProperty('image.scale.x', getProperty('image.scale.x') + 0.5);
		setProperty('image.scale.y', getProperty('image.scale.y') + 0.3);
		playSound('what', 1)
